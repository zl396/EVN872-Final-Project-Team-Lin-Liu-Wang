...
file_list <-
  list.files(
    '~/Documents/872_Final_Project/Processed',
    pattern = "*.csv",
    recursive = TRUE,
    full.names = TRUE
  )
file_name <- c('California','combined_df', 'Conecticut', 
...
plotting_function <- function(state_name){ggplot(data_wrangler(state_name), 
    ...
    labs(y= 'Clean Energy Generation %', x='Date', title='Percentage of Clean Energy Generation (%)', subtitle = paste(state_name,'(2001.1-2023.8)'))+
    mytheme
  }
```
...
arima_ts <- function(state_name,t){
  ...
  clean.perc.forecast <- forecast(clean.perc.arima, level=c(95), h=t*12)
  autoplot(clean.perc.forecast)+
    labs(y= 'Clean Energy Generation %', x='Date', title='Percentage of Clean Energy Generation Forecast (%)', subtitle = paste(state_name,'(2001.1-2023.8)'))+
    mytheme
}
```
### California 
